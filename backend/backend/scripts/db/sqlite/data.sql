-- PetClinic seed data, SQLite-compatible.
--
-- Differences from the MySQL version:
--   * MySQL boolean literal `TRUE` -> `1`. (SQLite >= 3.23 accepts TRUE as an
--     alias for 1, but `1` works on every SQLite version and matches how
--     BOOLEAN columns are actually stored, since SQLite has no native boolean.)
--   * Wrapped in a transaction. SQLite commits per-statement by default, which
--     makes batch inserts ~100x slower; one BEGIN/COMMIT pair is much faster.
--
-- Everything else (ISO date strings, explicit INTEGER PRIMARY KEY values,
-- positional VALUES) is already valid SQLite.
...
INSERT INTO users (username, password, enabled) VALUES
    ('admin', '$2a$10$ymaklWBnpBKlgdMgkjWVF.GMGyvH8aDuTK.glFOaKw712LHtRRymS', 1);
